//! WorkspaceHost — owns the local `WorkspaceDoc`: snapshot persistence, the
//! device registry row for THIS device, and the typed watch channels the
...
impl WorkspaceHost {
    ...
    pub fn set_chat_config(&self, chat_id: &str, config: &ChatConfig) -> Result<bool, EngineError> {
        let previous = self.inner.doc.chat(chat_id)?;
        let changed = self.inner.doc.set_chat_config(chat_id, config)?;
        // Switching harness mid-chat must not resume with the previous
        // provider's session id (Claude session ≠ Codex session ≠ OpenAI).
        if changed
            && let Some(chat) = previous
            && let Some(prev) = chat.config.as_ref()
            && prev.harness != config.harness
        {
            self.inner.doc.set_chat_harness_session(chat_id, "", "")?;
        }
        Ok(changed)
    }
...
/// re-publishing the watch channels and debouncing snapshots. Holds only a weak
