...
use comet_proto::{
    AgentEvent, DocumentRef, DoneStatus, HarnessId, Model, ReasoningLevel, RunRequest,
...
#[tokio::test]
async fn document_refs_survive_queue_command_to_harness() {
    let dir = tempfile::tempdir().unwrap();
    let seen: Arc<std::sync::Mutex<Vec<RunRequest>>> = Default::default();
    let core = assemble(
        dir.path(),
    ...
    // Mirrors the Aurin composer after an @ mention: the user text carries the
    // markdown token and the structured ref rides the additive field.
    let mut request = run_request("请在这个文档里加一个代码块");
    request.document_refs = vec![DocumentRef {
    ...
    let chat_run = requests
        .iter()
        .find(|r| r.prompt.contains("加一个代码块"))
        .expect("chat run reached the harness");
    assert_eq!(
        chat_run.document_refs,
        vec![DocumentRef {
            node_id: "019fbc42-9b06-70a3-ad29-563606bc172a".into(),
            content_id: "019fbc42-9b06-70a3-ad29-564a1bd00e48".into(),
            title: "你好我有一个帽衫".into(),
