...
impl EngineHandle {
    /// Build a handle from an already-connected RPC client (hosts like Aurin
    /// can wrap their in-process engine with `memory_client` instead of
    /// round-tripping through an IPC port).
    pub fn from_client(client: RpcClient, url: String) -> Self {
        Self {
...
impl AppState {
    ...
    /// Host-presence check. The local note-agent build is single-device, so
    /// every registered device is treated as reachable.
    pub fn device_online(&self, _device_id: &str, _now: DateTime<Utc>) -> bool {
        true
    }

    /// Git detection is gone; spaces are plain local folders.
