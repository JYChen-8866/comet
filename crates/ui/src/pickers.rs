...
impl Pickers {
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        let search_events = cx.subscribe(&search, |this: &mut Self, _, event, cx| match event {
            ...
            ComposerInputEvent::MentionRequested { .. } => {}
        });
    ...
    /// The chat interface lets the user switch providers mid-conversation, so
    /// the harness is never locked.
    ...
    fn effective_harness(&self, cx: &App) -> Option<HarnessId> {
        ...
        {
            // A stale `Mock` config row (created before a real provider was
            // configured) should not win; fall through to the catalog.
            if config.harness != HarnessId::Mock {
...
pub(crate) fn harness_brand_icon(harness: HarnessId) -> (&'static str, Option<gpui::Hsla>) {
    match harness {
        ...
        HarnessId::OpenAiCompatible => (crate::icons::OPENAI_MARK, None),
        HarnessId::DeepSeek => (crate::icons::DEEPSEEK_MARK, None),
...
fn visible_harnesses_impl(list: &[HarnessDescriptor], allow_mock: bool) -> Vec<HarnessDescriptor> {
    ...
    let mut real: Vec<HarnessDescriptor> = list
        .iter()
        .filter(|d| d.id != HarnessId::Mock)
        .cloned()
        .collect();
    // When a host app (Aurin) registers an OpenAI-compatible provider, make it
    // the default rail row; Claude/Codex stay selectable.
    if let Some(pos) = real.iter().position(|d| d.id == HarnessId::OpenAiCompatible) {
        let openai = real.remove(pos);
        real.insert(0, openai);
    }
