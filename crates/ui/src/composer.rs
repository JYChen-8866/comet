...
use comet_proto::{DocumentRef, RunRequest, SandboxLevel, UserInputAnswer, UserInputQuestion};
...
fn document_mentions(text: &str) -> Vec<DocumentMention> {
    ...
    for (start, _) in text.match_indices("@[") {
        ...
        let Some(marker_start_rel) = text[title_end..].find(MARKER) else {
            continue;
        };
        let id_start = title_end + marker_start_rel + MARKER.len();
...
impl EntityInputHandler for ComposerInput {
    ...
    fn replace_and_mark_text_in_range(
        ...
        self.reset_blink();
        if self.content[..range.start + new_text.len()].ends_with('@') {
...
impl Composer {
    ...
    /// Insert text at the caret (host mention picker result).
