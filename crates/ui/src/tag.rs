//! Tag module.
//!
//! Copied 1:1 from liora-components (`crates/liora-components/src/tag.rs`),
//! adapted to comet's theme tokens and embedded icon assets.

use gpui::{
    AnyElement, App, Component, ElementId, IntoElement, Pixels, RenderOnce, SharedString, Window,
    div, prelude::*, px,
};
...
/// Fluent native GPUI component for rendering a tag/chip.
pub struct Tag {
    label: SharedString,
...
impl RenderOnce for Tag {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let (bg, border, text_color) = match self.effect {
            ...
            TagEffect::Dark => (color, color, theme.text.opacity(1.0)),
            TagEffect::Plain => (theme.surface, color.opacity(0.4), color),
        };
...
/// Fluent native GPUI component for rendering tag flow.
pub struct TagFlow {
    tags: Vec<AnyElement>,
    gap: Pixels,
    align: TagFlowAlign,
...
fn element_id(id: impl Into<SharedString>) -> ElementId {
