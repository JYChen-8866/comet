//! The app shell: sidebar column + main panel, plus the boot splash and the
//! connection gate.
//!
...
//! shifts. Widths/collapsed state persist to `ui-settings.json` (debounced).
//!
...
use crate::settings::{
    KeymapConfig, SAVE_DEBOUNCE_MS, SIDEBAR_MAX, SIDEBAR_MIN, UiSettings, platform_combo,
};
use crate::state::{
...
/// Key bindings comet itself installed in the previous [`apply_keymap`] call.
/// `apply_keymap` is the only path that clears the app keymap, so it can
/// snapshot every binding before the clear and restore the ones that belong
/// to the host application (framework components, the embedded editor, ...)
/// afterwards, dropping only the shortcuts comet installed last time.
thread_local! {
    static COMET_KEY_BINDINGS: RefCell<Vec<KeyBinding>> = const { RefCell::new(Vec::new()) };
}

/// Structural equality for [`KeyBinding`]: same keystrokes, same action (name
/// and input), same context predicate. Used to tell comet's own bindings apart
/// from host-contributed ones in [`apply_keymap`].
fn binding_matches(left: &KeyBinding, right: &KeyBinding) -> bool {
...
pub fn apply_keymap(cx: &mut App, keymap: &KeymapConfig) {
    ...
    // Snapshot everything currently bound (host framework bindings such as
    // gpui-component's inputs, the embedded editor keymap, ...) and drop only
    ...
    // Remember what comet just installed so the next application can drop the
    // old shortcut combos without touching host bindings.
    let installed: Vec<KeyBinding> = cx.key_bindings().borrow().bindings().cloned().collect();
    COMET_KEY_BINDINGS.with(|slot| *slot.borrow_mut() = installed);

    // Restore the host's bindings after comet's own keymap.
    cx.bind_keys(preserved);
}
...
pub struct Shell {
    ...
    route: Route,
    /// The chat currently shown ("" = the new-chat canvas) — route history key.
    active_chat: String,
    ...
    /// Chat-only build: one default local space is created on first boot so
    /// new chats work without the sidebar's add-space flow.
    default_space_started: bool,
    default_space_task: Option<Task<()>>,
...
impl Shell {
    pub fn new(state: Entity<AppState>, boot: EngineBootConfig, cx: &mut Context<Self>) -> Self {
        ...
        let route = match std::env::var("COMET_OPEN_ROUTE").ok().as_deref() {
            Some("settings") => Route::Settings(SettingsSection::Shortcuts),
        ...
        let debug_gate = match std::env::var("COMET_FORCE_GATE").ok().as_deref() {
            Some("signin") => Some(GatePhase::Failed("sign-in removed".into())),
            Some("org") => Some(GatePhase::Failed("org gate removed".into())),
        ...
        Self {
            ...
            shortcuts_page: None,
            shortcuts_sub: None,
            default_space_started: false,
            default_space_task: None,
            chat_menu: None,
            rename_dialog: None,
            delete_confirm: None,
            space_menu: None,
    ...
    /// Build the same shell in sidebar-only mode: no titlebar, no main chat
    /// column — just the original comet session-management sidebar.
    pub fn new_sidebar(
        state: Entity<AppState>,
        boot: EngineBootConfig,
        cx: &mut Context<Self>,
    ...
    fn on_state_changed(&mut self, state: &Entity<AppState>, cx: &mut Context<Self>) {
        // Chat-only build: auto-create the default local space once the engine
        // is ready and no space exists (the sidebar's add-space flow is gone).
        let (needs_default_space, local_device_id) = {
            let state = state.read(cx);
            (
                matches!(state.connection, ConnectionStatus::Ready)
                    && !self.default_space_started
                    && state.local_device_id.is_some()
                    && state.spaces.is_empty(),
                state.local_device_id.clone(),
            )
        };
        if needs_default_space {
            self.default_space_started = true;
            if let Some(device_id) = local_device_id {
                self.ensure_default_space(device_id, cx);
            }
        }
    ...
    fn sidebar_target(&self) -> f32 {
        // Sidebar removed for the chat-only build: the chat card fills the
        // window, and the MessageRail outline lives inside the transcript.
        0.0
    }
    ...
    fn ensure_default_space(&mut self, device_id: String, cx: &mut Context<Self>) {
        ...
        self.default_space_task = Some(cx.spawn(async move |this, cx| {
            ...
            if let Err(err) = engine.client().call(methods::MUTATE, params).await {
                tracing::warn!(error = %err, "default space creation failed; will retry");
                this.update(cx, |this, _| this.default_space_started = false)
                    .ok();
            }
        }));
    }
    ...
    /// New-session affordance for the embedded sidebar: switch to the new-chat
    /// canvas in the current space (the composer mints the chat on first send).
    ...
    /// Chat-mode sidebar for the host panel: a New session button and the
    /// global Sessions list. No spaces section, no user menu.
    fn render_chat_sidebar(&mut self, theme: &Theme, cx: &mut Context<Self>) -> AnyElement {
        ...
        sidebar
            ...
            // The New session button + the global Sessions list share one
            // scroll region. On glass the whole region paints inside an
            // EdgeFade scope — a true per-glyph gradient at active edges.
    ...
    /// UserMenu: local trigger row; menu with Open settings.
    fn render_user_menu(
        &mut self,
        ...
        // Bottom-of-sidebar identity: avatar circle + name with a quiet
        // "Local" line underneath.
...
impl Render for Shell {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let theme = Theme::of(cx);
        if self.sidebar_only {
            let theme = theme.clone();
            let sidebar = self.render_chat_sidebar(&theme, cx);
            return div()
        ...
        let root = match &gate {
            GatePhase::Ready => {
                ...
                // The signature frame: one rounded hairline-bordered card on
                // the frost shell.
                let theme = Theme::of(cx);
