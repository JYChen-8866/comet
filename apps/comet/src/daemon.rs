...
//! a systemd **user** unit on Linux, a launchd LaunchAgent on macOS. The unit
//! runs the current executable with the `COMET_*` environment captured at
//! install time.
...
/// Same unit name the packaging installers write, so `comet daemon …` manages
/// that installation rather than a competing copy.
const SYSTEMD_UNIT: &str = "comet-native.service";

/// Environment captured into the unit file. The `COMET_*`/logging vars only
/// when set.
...
fn render_systemd_unit(exe: &Path, env: &[(String, String)]) -> String {
    // The start limit trips on a fail-fast exit (5 × RestartSec=5 lands inside
    // the 60s window) — otherwise a misconfigured daemon restart-loops forever.
...
mod tests {
    ...
    fn systemd_unit_shape() {
        let unit = render_systemd_unit(
            ...
            &[
                ...
                ("COMET_DATA_DIR".into(), "/tmp/data".into()),
        ...
        assert!(unit.contains("Environment=\"COMET_DATA_DIR=/tmp/data\"\n"));
    ...
    fn launchd_plist_shape() {
        let plist = render_launchd_plist(
            ...
            &[("COMET_DATA_DIR".into(), "/Users/x/a&b".into())],
        ...
        assert!(plist.contains("<string>/Users/x/a&amp;b</string>"));
