...
> Refactored for the local note-agent build: account auth (WorkOS), edge cloud
> sync, the device relay, self-update, external coding-agent harnesses,
> terminals, and repos/diffs have been removed. The rest of this document
> describes the original multi-device design and is kept as history; the
> current product is a local engine + desktop UI (see `README.md`).
